...
export default function P12Ch04Sec15({ currentTime, reveals, language }: SceneProps) {
  ...
  return (
    <svg viewBox="0 0 1080 620" preserveAspectRatio="xMidYMin meet" className="w-full h-full select-none">
      ...
      {/* BEAT 1, 3 & 5: JEE Main Coaxial Cable */}
      <Badge n={1} cx={52} cy={140} on={beat >= 1} delay={dl(1, 0.4)} />
      <Fade on={beat >= 1} delay={dl(1, 0.8)}>
        <T x={74} y={145} size={14} fill={RED} weight={700} anchor="start">JEE MAIN: COAXIAL CABLE FIELD</T>
      </Badge>
      <Fade on={beat >= 1} dim={beat >= 7}>
        <g transform="translate(60, 160)">
          <rect x={0} y={5} width={450} height={60} rx={8} fill={CREAM} stroke={AMBER_DARK} strokeWidth={1.8} />
          <T x={225} y={30} anchor="middle" size={15} fill={INK} weight={800}>
            Between Conductors: B = 0.60 mT  (I_enc = 6.0A)
          </T>
          <T x={225} y={52} anchor="middle" size={16} fill={GREEN} weight={800}>
            Outside Cable: B = 0  (I_enc = 6.0 - 6.0 = 0!)
          </T>
        </g>
      </Fade>

      {/* BEAT 7, 8 & 12, 13: JEE Advanced Drilled Cavity */}
      <Badge n={2} cx={540} cy={140} on={beat >= 7} delay={dl(7, 0.4)} />
      <Fade on={beat >= 7} delay={dl(7, 0.8)}>
        <T x={562} y={145} size={14} fill={RED} weight={700} anchor="start">JEE ADVANCED: UNIFORM FIELD IN DRILLED CAVITY</T>
      </Badge>
      <Fade on={beat >= 7} dim={beat >= 14}>
        <g transform="translate(540, 160)">
          <rect x={0} y={5} width={480} height={60} rx={8} fill={CREAM} stroke={GREEN} strokeWidth={1.8} />
          <T x={240} y={30} anchor="middle" size={15} fill={INK} weight={800}>
            \vec{B} = (μ₀/2) \vec{J} × \vec{r}₁ + (μ₀/2) (−\vec{J}) × \vec{r}₂
          </T>
          <T x={240} y={52} anchor="middle" size={17} fill={GREEN} weight={800}>
            ⇒  \vec{B}_{\text{cavity}} = (μ₀ / 2) \vec{J} × \vec{d}  (PERFECTLY UNIFORM!)
          </T>
        </g>
      </Fade>
      ...
      <Fade on={beat >= 14}>
        <Chip x={100} y={480} w={880} h={44} fill={GREEN} textFill="#ffffff" size={18}>
          {t(
            ...
            "★ Coaxial cable field confine karta hai B_out = 0! Drilled cavity mein uniform field B = ½ μ0 J d hai! ✓"
          )}
        </Chip>
      </Fade>
    </svg>
