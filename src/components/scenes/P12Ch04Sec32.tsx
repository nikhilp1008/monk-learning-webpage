...
export default function P12Ch04Sec32({ currentTime, reveals, language }: SceneProps) {
  ...
  return (
    <svg viewBox="0 0 1080 620" preserveAspectRatio="xMidYMin meet" className="w-full h-full select-none">
      ...
      {/* BEAT 1 & 3: CBSE Ammeter Shunt Numerical */}
      <Badge n={1} cx={52} cy={140} on={beat >= 1} delay={dl(1, 0.4)} />
      <Fade on={beat >= 1} delay={dl(1, 0.8)}>
        <T x={74} y={145} size={14} fill={RED} weight={700} anchor="start">CBSE LEVEL: AMMETER SHUNT NUMERICAL</T>
      </Badge>
      <Fade on={beat >= 1} dim={beat >= 5}>
        <g transform="translate(60, 160)">
          <rect x={0} y={5} width={450} height={60} rx={8} fill={CREAM} stroke={AMBER_DARK} strokeWidth={1.8} />
          <T x={225} y={30} anchor="middle" size={15} fill={INK} weight={800}>
            G = 50 Ω, I_g = 2.0 mA, I = 5.0 A
          </T>
          <T x={225} y={52} anchor="middle" size={16} fill={GREEN} weight={800}>
            S = (0.002 × 50) / 4.998 = 0.0200 Ω  (In parallel!)
          </T>
        </g>
      </Fade>

      {/* BEAT 5, 7 & 8: NEET Minus-G Voltmeter Speed Trap */}
      <Badge n={2} cx={540} cy={140} on={beat >= 5} delay={dl(5, 0.4)} />
      <Fade on={beat >= 5} delay={dl(5, 0.8)}>
        <T x={562} y={145} size={14} fill={RED} weight={700} anchor="start">NEET SPEED TRAP: MINUS-G CORRECTION</T>
      </Badge>
      <Fade on={beat >= 5} dim={beat >= 9}>
        <g transform="translate(540, 160)">
          <rect x={0} y={5} width={480} height={60} rx={8} fill={CREAM} stroke={GREEN} strokeWidth={1.8} />
          <T x={240} y={30} anchor="middle" size={15} fill={INK} weight={800}>
            R = (V / I_g) − G = (3.0 / 0.001) − 60
          </T>
          <T x={240} y={52} anchor="middle" size={17} fill={GREEN} weight={800}>
            R = 3000 − 60 = 2940 Ω  (In series!)
          </T>
        </g>
      </Fade>
      ...
      <Fade on={beat >= 9}>
        <Chip x={100} y={480} w={880} h={44} fill={GREEN} textFill="#ffffff" size={18}>
          {t(
            ...
            "★ Result: Ammeter Shunt S = 0.0200 Ω (parallel) | Voltmeter Multiplier R = 2940 Ω (series, minus G karein!) ✓"
          )}
        </Chip>
      </Fade>
    </svg>
