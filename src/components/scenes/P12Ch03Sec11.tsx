...
export default function P12Ch03Sec11({ currentTime, reveals, language }: SceneProps) {
  ...
  return (
    <svg viewBox="0 0 1080 620" preserveAspectRatio="xMidYMin meet" className="w-full h-full select-none">
      ...
      {/* BEAT 1: Substitute v_d into I */}
      <Badge n={1} cx={52} cy={140} on={beat >= 1} delay={dl(1, 0.4)} />
      <Fade on={beat >= 1} delay={dl(1, 0.8)}>
        <T x={74} y={145} size={14} fill={RED} weight={700} anchor="start">SUBSTITUTE v_d INTO CURRENT</T>
      </Fade>
      <Fade on={beat >= 1} dim={beat >= 3}>
        <g transform="translate(60, 160)">
          <rect x={0} y={5} width={450} height={50} rx={8} fill={CREAM} stroke={AMBER_DARK} strokeWidth={1.8} />
          <T x={225} y={35} anchor="middle" size={18} fill={INK} weight={800}>
            I = n e A v_d = n e A (e E τ / m)
          </T>
        </g>
      </Fade>

      {/* BEAT 3: Substitute E = V / L */}
      <Badge n={2} cx={540} cy={140} on={beat >= 3} delay={dl(3, 0.4)} />
      <Fade on={beat >= 3} delay={dl(3, 0.8)}>
        <T x={562} y={145} size={14} fill={RED} weight={700} anchor="start">REPLACE E = V / L</T>
      </Fade>
      ...
      {/* BEAT 5: Rearrange V = IR */}
      <Badge n={3} cx={52} cy={270} on={beat >= 5} delay={dl(5, 0.4)} />
      <Fade on={beat >= 5} delay={dl(5, 0.8)}>
        <T x={74} y={275} size={14} fill={RED} weight={700} anchor="start">REARRANGE FOR V = I R</T>
      </Fade>
      <Fade on={beat >= 5} dim={beat >= 7}>
        <g transform="translate(60, 290)">
          ...
          <T x={480} y={52} anchor="middle" size={15} fill={INK} weight={800} script>
            {t(
              "Where Resistivity ρ = m / (n e² τ)",
              "Jahan Resistivity ρ = m / (n e² τ) hoti hai"
            )}
          </T>
        </g>
      </Fade>
      ...
      <Fade on={beat >= 7}>
        <Chip x={100} y={480} w={880} h={44} fill={GREEN} textFill="#ffffff" size={18}>
          {t(
            ...
            "★ Derived! R = (mL)/(ne²τA) aur ρ = m/(ne²τ). Ohm's law electron dynamics se banta hai! ✓"
          )}
        </Chip>
      </Fade>
    </svg>
