...
export default function P12Ch03Sec34({ currentTime, reveals, language }: SceneProps) {
  ...
  return (
    <svg viewBox="0 0 1080 620" preserveAspectRatio="xMidYMin meet" className="w-full h-full select-none">
      ...
      {/* BEAT 1 & 3: Node Potential Difference */}
      <Badge n={1} cx={52} cy={140} on={beat >= 1} delay={dl(1, 0.4)} />
      <Fade on={beat >= 1} delay={dl(1, 0.8)}>
        <T x={74} y={145} size={14} fill={RED} weight={700} anchor="start">SHARED NODES (SAME VOLTAGE V)</T>
      </Fade>
      <Fade on={beat >= 1} dim={beat >= 4}>
        <g transform="translate(60, 160)">
          ...
          <T x={225} y={35} anchor="middle" size={15} fill={INK} weight={800} script>
            {t(
              "Both ends connected to same two nodes  ⇒  Same voltage V across each branch",
              "Dono end same nodes se jude hain  ⇒  Har branch pe same voltage V"
            )}
          </T>
        </g>
      </Fade>

      {/* BEAT 4 & 5: Total Current Sum */}
      <Badge n={2} cx={540} cy={140} on={beat >= 4} delay={dl(4, 0.4)} />
      <Fade on={beat >= 4} delay={dl(4, 0.8)}>
        <T x={562} y={145} size={14} fill={RED} weight={700} anchor="start">KIRCHHOFF CURRENT SUM</T>
      </Fade>
      ...
      {/* BEAT 6: 1/R_p Derivation */}
      <Badge n={3} cx={52} cy={270} on={beat >= 6} delay={dl(6, 0.4)} />
      <Fade on={beat >= 6} delay={dl(6, 0.8)}>
        <T x={74} y={275} size={14} fill={RED} weight={700} anchor="start">CURRENT DIVIDER DERIVATION</T>
      </Fade>
      <Fade on={beat >= 6}>
        <g transform="translate(60, 290)">
          ...
          <T x={480} y={52} anchor="middle" size={16} fill={GREEN} weight={800}>
            Two branches: V = I [ (R₁ R₂) / (R₁ + R₂) ]   ⇒   I₁ = V / R₁ = I [ R₂ / (R₁ + R₂) ]
          </T>
        </g>
      </Fade>
      ...
      <Fade on={beat >= 6}>
        <Chip x={100} y={480} w={880} h={44} fill={GREEN} textFill="#ffffff" size={18}>
          {t(
            ...
            "★ Derived! Current divider I₁ = I [ R₂ / (R₁ + R₂) ] Ohm's Law se aata hai! ✓"
          )}
        </Chip>
      </Fade>
    </svg>
