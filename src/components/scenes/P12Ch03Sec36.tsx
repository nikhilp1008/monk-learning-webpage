    ...
    <svg viewBox="0 0 1080 620" preserveAspectRatio="xMidYMin meet" className="w-full h-full select-none">
      ...
      <Fade on={beat >= 1} delay={dl(1, 0.3)}>
        <T x={60} y={120} size={13} fill={MUTED} anchor="start" script>
          {t(
            "Four resistors of 12 Ω each. Find R_max and R_min using all four.",
            "Four resistors of 12 Ω each. Sabhi charo ko mila kar R_max aur R_min nikaalein."
          )}
        </T>
      </Fade>

      {/* BEAT 3 & 4: Pure Series Max */}
      <Badge n={1} cx={52} cy={160} on={beat >= 3} delay={dl(3, 0.4)} />
      <Fade on={beat >= 3} delay={dl(3, 0.8)}>
        <T x={74} y={165} size={14} fill={RED} weight={700} anchor="start">PURE SERIES (MAXIMUM)</T>
      </Badge>
      ...
      {/* BEAT 5: Pure Parallel Min */}
      <Badge n={2} cx={540} cy={160} on={beat >= 5} delay={dl(5, 0.4)} />
      <Fade on={beat >= 5} delay={dl(5, 0.8)}>
        <T x={562} y={165} size={14} fill={RED} weight={700} anchor="start">PURE PARALLEL (MINIMUM)</T>
      </Badge>
      ...
      <Fade on={beat >= 6} delay={dl(6, 0.8)}>
        <T x={74} y={295} size={14} fill={RED} weight={700} anchor="start">RATIO CHECK R_max / R_min</T>
      </Fade>
      <Fade on={beat >= 6}>
        <g transform="translate(60, 310)">
          <rect x={0} y={5} width={960} height={50} rx={8} fill={CREAM} stroke={GREEN} strokeWidth={1.8} />
          <T x={480} y={35} anchor="middle" size={16} fill={GREEN} weight={800}>
            R_max / R_min = 48 / 3 = 16 = 4² = n²   (Confirms both answers in 1 line!)
          </T>
        </g>
      </Fade>
      ...
      <Fade on={beat >= 7}>
        <Chip x={100} y={480} w={880} h={44} fill={GREEN} textFill="#ffffff" size={18}>
          {t(
            ...
            "★ Result: R_max = 48 Ω, R_min = 3 Ω. Extremes HAMESHA pure series aur pure parallel hote hain! ✓"
          )}
        </Chip>
      </Fade>
    </svg>
