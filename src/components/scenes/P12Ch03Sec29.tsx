  ...
  return (
    <svg viewBox="0 0 1080 620" preserveAspectRatio="xMidYMin meet" className="w-full h-full select-none">
      ...
      <Fade on={beat >= 6} delay={dl(6, 0.8)}>
        <T x={74} y={295} size={14} fill={RED} weight={700} anchor="start">PREDICT BEFORE COMPUTING</T>
      </Fade>
      <Fade on={beat >= 6}>
        <g transform="translate(60, 310)">
          ...
          <T x={480} y={35} anchor="middle" size={15} fill={INK} weight={800} script>
            {t(
              "If parallel result > min(R_i), stop and check your math immediately!",
              "Agar parallel result min(R_i) se bada aata hai, to calculation turant check karein!"
            )}
          </T>
        </g>
      </Fade>
      ...
      <Fade on={beat >= 7}>
        <Chip x={100} y={480} w={880} h={44} fill={GREEN} textFill="#ffffff" size={18}>
          {t(
            ...
            "★ n identical resistors ke liye R_s / R_p = n². Hamesha R_p < R_min check karein! ✓"
          )}
        </Chip>
      </Fade>
    </svg>
