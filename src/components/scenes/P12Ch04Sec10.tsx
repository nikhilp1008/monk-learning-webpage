...
export default function P12Ch04Sec10({ currentTime, reveals, language }: SceneProps) {
  ...
  return (
    <svg viewBox="0 0 1080 620" preserveAspectRatio="xMidYMin meet" className="w-full h-full select-none">
      ...
      {/* BEAT 1 & 3: High Symmetry Requirement */}
      <Badge n={1} cx={52} cy={140} on={beat >= 1} delay={dl(1, 0.4)} />
      <Fade on={beat >= 1} delay={dl(1, 0.8)}>
        <T x={74} y={145} size={14} fill={RED} weight={700} anchor="start">THE HIGH-SYMMETRY REQUIREMENT</T>
      </Fade>
      <Fade on={beat >= 1} dim={beat >= 4}>
        <g transform="translate(60, 160)">
          ...
          <T x={225} y={35} anchor="middle" size={15} fill={INK} weight={800} script>
            {t(
              "Law is ALWAYS valid, but solves B only when B is constant along Amperian path!",
              "Law hamesha valid hai, lekin B tabhi solve karta hai jab B path par constant ho!"
            )}
          </T>
        </g>
      </Fade>

      {/* BEAT 4 & 6: Four High-Symmetry Cases */}
      <Badge n={2} cx={540} cy={140} on={beat >= 4} delay={dl(4, 0.4)} />
      <Fade on={beat >= 4} delay={dl(4, 0.8)}>
        <T x={562} y={145} size={14} fill={RED} weight={700} anchor="start">THE FOUR CLASSIC APPLICATIONS</T>
      </Fade>
      <Fade on={beat >= 4} dim={beat >= 7}>
        <g transform="translate(540, 160)">
          <rect x={0} y={5} width={480} height={60} rx={8} fill={CREAM} stroke={GREEN} strokeWidth={1.8} />
          <T x={240} y={30} anchor="middle" size={15} fill={INK} weight={800}>
            1. Infinite Wire  |  2. Thick Cylinder/Coaxial
          </T>
          <T x={240} y={52} anchor="middle" size={15} fill={GREEN} weight={800}>
            3. Long Solenoid  |  4. Toroid Loop
          </T>
        </g>
      </Fade>
      ...
      <Fade on={beat >= 6}>
        <Chip x={100} y={480} w={880} h={44} fill={GREEN} textFill="#ffffff" size={18}>
          {t(
            ...
            "★ Ampere's Law ke liye steady currents & symmetric Amperian loops (wire, solenoid, toroid) chahiye! ✓"
          )}
        </Chip>
      </Fade>
    </svg>
