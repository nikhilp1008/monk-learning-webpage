...
import { startDoubtOfDaySession, endSession } from "@/lib/drona/client";
import {
  DEFAULT_LANGUAGE,
  getTutorName,
...
interface DoubtChatOverlayProps {
  doubtId: string;
  /** Shown as the session title while turn 1 is still being generated. */
  concept?: string | null;
  onClose: () => void;
}

/**
 * Full-screen overlay on top of /dashboard — a two-minute chat about the
 * doubt of the day. Same socket lifecycle as PracticeExplainOverlay (both are
 * disposable single-segment sessions that deliberately sit outside the
 * ACTIVE_SESSION_KEY resume mechanism); the difference is entirely in the
 * server-side prompt, which withholds the answer until the student has had a
 * go at it themselves.
 */
export function DoubtChatOverlay({ doubtId, concept, onClose }: DoubtChatOverlayProps) {
  ...
  const handleEndChat = useCallback(() => {
    ...
    if (sessionId) {
      ...
      // call isn't worth blocking the student's return to the dashboard over.
      endSession(sessionId).catch((err) => console.warn("End doubt-of-day session failed:", err));
    }
    onClose();
  }, [sessionId, onClose]);
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        const res = await startDoubtOfDaySession(
          doubtId,
          loadLanguagePreference(),
          loadVoicePreference()
        );
        ...
        if (cancelled) return;
        console.error("Failed to start doubt-of-day session:", err);
  ...
  // Voice socket lifecycle — mirrors PracticeExplainOverlay's effect.
  useEffect(() => {
    if (!sessionId) return;
    ...
    client.connect().catch((err) => console.warn("Doubt-of-day voice WS connect failed:", err));
    ...
    <div className="fixed inset-0 z-50 bg-ruled-body overflow-y-auto">
      {connecting ? (
        ...
        <main className="max-w-[1180px] w-full mx-auto px-0 md:px-6" style={{ paddingTop: 0 }}>
          <SessionView
            sessionTopic={concept ? `Today's doubt · ${concept}` : "Today's doubt"}
            tutorName={tutorName}
            language={language}
            boardLatex=""
